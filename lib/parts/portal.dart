...
class Ball extends PositionComponent with HasGameRef<RotatingArrowGame> {
  ...
  void updatePosition(double dt) {
    if (!hit) {
      radius += gameRef.arrow.speed.abs() * radialSpeedRatio * dt;
      angle += gameRef.arrow.speed * angularSpeedRatio * dt;
      position = center + Vector2(cos(angle), sin(angle)) * radius;
      trailTimer += dt;
      if (trailTimer >= 0.010 &&
    ...
    if (!hit &&
        ((gameRef.currentLevel >= 13 && gameRef.currentLevel <= 20) ||
            gameRef.currentLevel > 36)) {
